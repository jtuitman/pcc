...
push_to_R:=function(f,R)
  ...
  for i:=1 to #C do
    ...
    for j:=1 to #D do
      E:=Coefficients(D[j]);
      for k:=1 to #E do
        g:=g+(O!Coefficients(E[k]))*(Ox.1)^(k-1)*(S.1)^(j-1+v)*(R.1)^(i-1);
      end for;
    end for;
  end for;
  return g;
...
push_to_S:=function(f,S)
  ...
  for i:=1 to #C do
    D:=Coefficients(C[i]);
    for j:=1 to #D do
      g:=g+(O!Coefficients(D[j]))*Ox.1^(j-1)*(S.1)^(i-1+v);
    end for;
  end for;
  return g;
end function;
...
Zaxy_to_R:=function(f,R);
  ...
  S:=BaseRing(R); Ox:=BaseRing(S); O:=BaseRing(Ox); C:=Coefficients(f);
  g:=R!0;
  for i:=1 to #C do
    D:=Coefficients(C[i]);
    for j:=1 to #D do
      g:=g+(O!Coefficients(D[j]))*Ox.1^(j-1)*R.1^(i-1);  
    end for;
  end for;
  return g;
end function;
...
xpforx:=function(f,xp,r,m); 
  ...
  for i:=2 to degxf do
    xppow:=Append(xppow,radix_reduce(xppow[i]*xppow[2],r,m));
  end for;
